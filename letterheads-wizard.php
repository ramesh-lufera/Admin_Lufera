...
<div class="dashboard-main-body">
    <div class="d-flex flex-wrap align-items-center gap-3 mb-24 justify-content-between">
        <div class="d-flex align-self-end">
            ...
            <h6 class="fw-semibold mb-0">Letterheads Wizard</h6>
        </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['inline_update'])) {
    ...
    // Find existing record for this exact user + website + template
    $jsonId = 0;
    $json_str = null;
    ...
    if (
        ...
        // Normal text / select / email / textarea etc.
...
<!-- ════════════════════════════════════════════════════════════ -->
<h5>Letterheads A4 – 100 Nos Requirements</h5>

<?php

renderFieldExtended('company_name', $savedData, $user_role, 'Company Name', '', 'text');

renderFieldExtended('logo', $savedData, $user_role, 'Company Logo', '', 'file');
...
renderFieldExtended('company_address', $savedData, $user_role, 'Company Address', '', 'textarea');

renderFieldExtended('paper_type', $savedData, $user_role, 'Paper Type', '', 'select', [
    '100 GSM',
    '120 GSM',
    '170 GSM',
    'Custom'
...
renderFieldExtended('printing_type', $savedData, $user_role, 'Printing Type', '', 'select', [
    'Single Color',
    'Multi Color',
    'Full Color'
]);
...
$('#exportPdfBtn').click(function() {
            ...
            const sections = [
                {
                    title: 'Letterheads A4 – 100 Nos Requirements',
                    fields: [
                        ...
                        { label: 'Paper Type', key: 'paper_type' },
                        { label: 'Printing Type', key: 'printing_type' },
                        { label: 'Additional Requirements', key: 'additional_requirements' }
                    ]
                }
            ];

            let html = '<html><head><title>Letterheads Wizard</title>';
            html += '<style>';
    ...
    document.querySelectorAll('.load-record').forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            ...
            const fieldMap = {
                ...
                'printing_type':            data.printing_type?.value,
