...
<div class="dashboard-main-body">
    <div class="d-flex flex-wrap align-items-center gap-3 mb-24 justify-content-between">
        <div class="d-flex align-self-end">
            ...
            <h6 class="fw-semibold mb-0">Lufera Cart Wizard</h6>
        </div>
...
<?php
    ...
    while ($r = $result->fetch_assoc()) {
        ...
        if ($decoded && !empty($decoded['store_name']['value'] ?? '')) {
            $prevRecords[] = [
                'id'           => $r['id'],
                'data'         => $decoded,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['inline_update'])) {
    ...
    // ------------------------------------------------
    // Find the correct JSON record
    // Same identifying keys used by SAVE logic:
    ...
    // ------------------------------------------------
    // Every admin/user edit becomes pending
    // ------------------------------------------------
...
<!-- ════════════════════════════════════════════════════════════ -->
<h5>LuferaCart Store Requirements</h5>

<?php

renderFieldExtended('store_name', $savedData, $user_role, 'Store Name', '', 'text');

renderFieldExtended('website_url', $savedData, $user_role, 'Store Website URL', '', 'text');

renderFieldExtended('business_category', $savedData, $user_role, 'Business Category', '', 'text');

renderFieldExtended('estimated_products', $savedData, $user_role, 'Estimated Number of Products', '', 'number');

renderFieldExtended('payment_gateway', $savedData, $user_role, 'Preferred Payment Gateway', '', 'select', [
    'Razorpay',
    'Stripe',
    'PayPal',
    'PayU',
...
renderFieldExtended('shipping_required', $savedData, $user_role, 'Shipping Integration Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('inventory_management', $savedData, $user_role, 'Inventory Management Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('coupon_system', $savedData, $user_role, 'Coupon & Discount System', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('customer_accounts', $savedData, $user_role, 'Customer Login Accounts', '', 'select', [
    'Required',
    'Guest Checkout Only',
    'Both'
]);
...
$('#exportPdfBtn').click(function() {
            ...
            const sections = [
                {
                    title: 'LuferaCart Store Requirements',
                    fields: [
                        { label: 'Store Name', key: 'store_name' },
                        { label: 'Store Website URL', key: 'website_url' },
                        { label: 'Business Category', key: 'business_category' },
                        { label: 'Estimated Number of Products', key: 'estimated_products' },
                        { label: 'Preferred Payment Gateway', key: 'payment_gateway' },
                        { label: 'Shipping Integration Required?', key: 'shipping_required' },
                        { label: 'Inventory Management Required?', key: 'inventory_management' },
                        { label: 'Coupon & Discount System', key: 'coupon_system' },
                        { label: 'Customer Login Accounts', key: 'customer_accounts' },
                        { label: 'Additional Requirements', key: 'additional_requirements' }
                    ]
                }
            ];

            let html = '<html><head><title>Lufera Cart Wizard</title>';
            html += '<style>';
    ...
    document.querySelectorAll('.load-record').forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            ...
            const fieldMap = {
                ...
                'estimated_products':       data.estimated_products?.value,
                'payment_gateway':          data.payment_gateway?.value,
                'shipping_required':        data.shipping_required?.value,
                'inventory_management':     data.inventory_management?.value,
                'coupon_system':            data.coupon_system?.value,
                'customer_accounts':        data.customer_accounts?.value,
