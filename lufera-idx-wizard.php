...
<div class="dashboard-main-body">
    <div class="d-flex flex-wrap align-items-center gap-3 mb-24 justify-content-between">
        <div class="d-flex align-self-end">
            ...
            <h6 class="fw-semibold mb-0">Lufera IDX Wizard</h6>
        </div>
...
<?php
    ...
    while ($r = $result->fetch_assoc()) {
        ...
        if ($decoded && !empty($decoded['website_url']['value'] ?? '')) {
            $prevRecords[] = [
                'id'           => $r['id'],
                'data'         => $decoded,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['inline_update'])) {
    ...
    // ------------------------------------------------
    // Find existing JSON record using:
    ...
    // If no DB row exists, start with empty array.
    // ------------------------------------------------
    $data = [];
    ...
    // ------------------------------------------------
    // If field doesn't exist in JSON,
    // create the field automatically.
    // ------------------------------------------------
...
<!-- ════════════════════════════════════════════════════════════ -->
<h5>LuferaIDX Requirements</h5>

<?php

renderFieldExtended('website_url', $savedData, $user_role, 'Website URL', '', 'text');

renderFieldExtended('real_estate_company', $savedData, $user_role, 'Real Estate Company', '', 'text');

renderFieldExtended('idx_plan', $savedData, $user_role, 'Preferred IDX Plan', '', 'select', [
    'Agent',
    'Team',
    'Brokerage',
    'Enterprise'
...
renderFieldExtended('mls_feed_required', $savedData, $user_role, 'MLS Feed Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('number_of_agents', $savedData, $user_role, 'Number of Agent Users', '', 'number');

renderFieldExtended('lead_capture_forms', $savedData, $user_role, 'Lead Capture Forms Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('saved_search_alerts', $savedData, $user_role, 'Saved Searches & Alerts', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('crm_required', $savedData, $user_role, 'CRM Management Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('website_integration', $savedData, $user_role, 'Website Integration', '', 'select', [
    'New Website',
    'Existing Website'
]);
...
$('#exportPdfBtn').click(function() {
            ...
            const sections = [
                {
                    title: 'LuferaIDX Requirements',
                    fields: [
                        { label: 'Website URL', key: 'website_url' },
                        { label: 'Real Estate Company', key: 'real_estate_company' },
                        { label: 'Preferred IDX Plan', key: 'idx_plan' },
                        { label: 'MLS Feed Required?', key: 'mls_feed_required' },
                        { label: 'Number of Agent Users', key: 'number_of_agents' },
                        { label: 'Lead Capture Forms Required?', key: 'lead_capture_forms' },
                        { label: 'Saved Searches & Alerts', key: 'saved_search_alerts' },
                        { label: 'CRM Management Required?', key: 'crm_required' },
                        { label: 'Website Integration', key: 'website_integration' },
                        { label: 'Additional Requirements', key: 'additional_requirements' }
                    ]
                }
            ];

            let html = '<html><head><title>Lufera IDX Wizard</title>';
            html += '<style>';
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.load-record').forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            ...
            const fieldMap = {
                'website_url':             data.website_url?.value,
                'real_estate_company':     data.real_estate_company?.value,
                'idx_plan':                data.idx_plan?.value,
                'mls_feed_required':       data.mls_feed_required?.value,
                'number_of_agents':        data.number_of_agents?.value,
                'lead_capture_forms':      data.lead_capture_forms?.value,
                'saved_search_alerts':     data.saved_search_alerts?.value,
                'crm_required':            data.crm_required?.value,
                'website_integration':     data.website_integration?.value,
