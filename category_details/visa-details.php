...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        // Sanitize inputs
        $Access_your_marketing_at = $conn->real_escape_string($_POST['access_your_marketing_at'] ?? '');
        $Access_your_marketing_with_www = $conn->real_escape_string($_POST['access_your_marketing_with_www'] ?? '');
        $Marketing_ip_address = $conn->real_escape_string($_POST['marketing_ip_address'] ?? '');
        $Nameserver_1 = $conn->real_escape_string($_POST['nameserver_1'] ?? '');
        $Nameserver_2 = $conn->real_escape_string($_POST['nameserver_2'] ?? '');
        ...
        if ($result->num_rows > 0) {
            // Update only
            $updateSql = "UPDATE websites SET access_your_marketing_at=?, access_your_marketing_with_www=?, marketing_ip_address=?, nameserver_1=?, nameserver_2=? WHERE id=?";
            $stmt = $conn->prepare($updateSql);
            $stmt->bind_param("sssssi", $Access_your_marketing_at, $Access_your_marketing_with_www, $Marketing_ip_address, $Nameserver_1, $Nameserver_2, $websiteId);
            ...
            if ($stmt->execute()) {
                ...
                echo "<script>
                    Swal.fire({
                        icon: 'error',
                        title: 'Update Failed',
                        text: 'Error updating: " . addslashes($stmt->error) . "',
    ...
    $Access_your_marketing_at = $Access_your_marketing_with_www = $Marketing_ip_address = $Nameserver_1 = $Nameserver_2 = "";

    $sql = "SELECT access_your_marketing_at, access_your_marketing_with_www, marketing_ip_address, nameserver_1, nameserver_2 FROM websites WHERE id = ?";
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $Access_your_marketing_at = $data['access_your_marketing_at'];
        $Access_your_marketing_with_www = $data['access_your_marketing_with_www'];
        $Marketing_ip_address = $data['marketing_ip_address'];
...
<div class="dashboard-main-body">
    ...
    <form method="post" autocomplete="off">
        ...
        <div class="col-lg-6">
            <div class="card h-100 p-0">
                <div class="card-header border-bottom bg-base py-16 px-24">
                    <h6 class="text-lg fw-semibold mb-0">Visa Details:</h6>
                </div>
                <div class="card-body p-24 plan-details-shadow bg-base">
                <div class="d-flex justify-content-between my-3">
        ...
        <?php if ($role == '1' || $role == '2'): ?>
            <div class="col-lg-6 text-end">
                <button type="submit" class="lufera-bg lufera-text text-md px-56 py-11 radius-8" style="width: auto; height: fit-content;">Update</button>
            </div>
        <?php endif; ?>
    </form>
    </div>
