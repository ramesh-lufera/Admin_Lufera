...
<div class="dashboard-main-body">
    <div class="d-flex flex-wrap align-items-center gap-3 mb-24 justify-content-between">
        <div class="d-flex align-self-end">
            ...
            <h6 class="fw-semibold mb-0">Visiting Cards Wizard</h6>
        </div>
...
<?php
    ...
    while ($r = $result->fetch_assoc()) {
        ...
        if ($decoded && !empty($decoded['card_type']['value'] ?? '')) {
            $prevRecords[] = [
                'id'           => $r['id'],
                'data'         => $decoded,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['inline_update'])) {
    ...
    // Find existing JSON record for this exact user + website + template
    $jsonId = 0;
    $jsonStr = '';
    ...
    // If no database row exists, start with an empty array.
    $data = [];
    ...
    // If this field does not exist in JSON,
    ...
    // Every inline update becomes pending
...
<!-- ════════════════════════════════════════════════════════════ -->
<h5>Visiting Cards Requirements</h5>

<?php

renderFieldExtended('card_type', $savedData, $user_role, 'Visiting Card Type', '', 'select', [
    'Velvet Touch Visiting Cards – 500 Nos',
    'Standard Visiting Cards – 100 Nos',
    'Standard Visiting Cards – 1000 Nos'
]);

renderFieldExtended('name', $savedData, $user_role, 'Name', '', 'text');

renderFieldExtended('logo', $savedData, $user_role, 'Logo', '', 'file');

renderFieldExtended('designation', $savedData, $user_role, 'Designation', '', 'text');

renderFieldExtended('phone', $savedData, $user_role, 'Phone Number', '', 'text');

renderFieldExtended('cell', $savedData, $user_role, 'Mobile Number', '', 'text');

renderFieldExtended('website', $savedData, $user_role, 'Website', '', 'text');

renderFieldExtended('email', $savedData, $user_role, 'Email Address', '', 'email');

renderFieldExtended('printing_side', $savedData, $user_role, 'Printing Side', '', 'select', [
    'Single Side',
    'Double Side'
]);

renderFieldExtended('address', $savedData, $user_role, 'Address', '', 'textarea');
...
$('#exportPdfBtn').click(function() {
            ...
            const sections = [
                {
                    title: 'Visiting Cards Requirements',
                    fields: [
                        { label: 'Visiting Card Type', key: 'card_type' },
                        { label: 'Name', key: 'name' },
                        { label: 'Logo', key: 'logo' },
                        { label: 'Designation', key: 'designation' },
                        { label: 'Phone Number', key: 'phone' },
                        { label: 'Mobile Number', key: 'cell' },
                        { label: 'Website', key: 'website' },
                        { label: 'Email Address', key: 'email' },
                        { label: 'Printing Side', key: 'printing_side' },
                        { label: 'Address', key: 'address' }
                    ]
                }
            ];

            let html = '<html><head><title>Visiting Cards Wizard</title>';
            html += '<style>';
            ...
            html += '<h1>Visiting Cards Wizard</h1>';
    ...
    document.querySelectorAll('.load-record').forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            ...
            const fieldMap = {
                ...
                'printing_side':  data.printing_side?.value,
