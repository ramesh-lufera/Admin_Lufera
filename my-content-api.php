<?php
// public-packages-api.php
// Public API version of view-package (no login, CORS enabled)

// 1) CORS & no output before headers
header("Access-Control-Allow-Origin: *");
header("Access-Control-Allow-Headers: *");
header("Access-Control-Allow-Methods: GET, POST, OPTIONS");

// 2) DB connection only (no check_login)
include './partials/connection.php';

// 3) category param (optional). default 170
$product_category = isset($_GET['product_category']) ? intval($_GET['product_category']) : 164;

// 4) Fetch packages (same logic as your view-package.php)
$packages = [];
$durations = [];
...
if (!empty($package_data)) {
    ...
    // ONLY show Starter packages
    if (strtolower(trim($row['title'])) !== 'starter' &&
        strtolower(trim($row['package_name'])) !== 'starter') {
        continue; // skip non-Starter
    }

    $duration_name = $row['duration'];

    // Group Starter packages by duration
    $packages[$duration_name][] = $row;
...
// 6) Output HTML — include the same CSS/JS files your admin uses.
//    Adjust asset paths if your public web root differs.
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<!-- JS (match admin) -->
<script src="https://admin2.luferatech.com/assets/js/bootstrap.bundle.min.js"></script>
