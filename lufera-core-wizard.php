...
<div class="dashboard-main-body">
    <div class="d-flex flex-wrap align-items-center gap-3 mb-24 justify-content-between">
        <div class="d-flex align-self-end">
            ...
            <h6 class="fw-semibold mb-0">Lufera Core Wizard</h6>
        </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['inline_update'])) {
    ...
    // ------------------------------------------------
    // Find existing JSON record for this user,
    ...
    // If no record exists, start with empty JSON
    // ------------------------------------------------
    $data = [];
    ...
    // ------------------------------------------------
    // If the field does not exist in JSON,
    ...
    if (
        ...
        // Normal text/select/number/textarea value
...
<!-- ════════════════════════════════════════════════════════════ -->
<h5>LuferaCore Custom Application Requirements</h5>

<?php

renderFieldExtended('company_name', $savedData, $user_role, 'Company Name', '', 'text');

renderFieldExtended('application_name', $savedData, $user_role, 'Application Name', '', 'text');

renderFieldExtended('application_type', $savedData, $user_role, 'Application Type', '', 'select', [
    'ERP',
    'CRM',
    'HRMS',
    'Inventory',
...
renderFieldExtended('number_of_users', $savedData, $user_role, 'Expected Number of Users', '', 'number');

renderFieldExtended('required_modules', $savedData, $user_role, 'Required Modules', '', 'textarea');

renderFieldExtended('workflow_automation', $savedData, $user_role, 'Workflow Automation Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('api_integrations', $savedData, $user_role, 'Third-Party API Integrations', '', 'textarea');

renderFieldExtended('role_based_access', $savedData, $user_role, 'Role-Based Access Control?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('reports_dashboard', $savedData, $user_role, 'Reports & Dashboard Required?', '', 'select', [
    'Yes',
    'No'
]);
...
$('#exportPdfBtn').click(function() {
            ...
            const sections = [
                {
                    title: 'LuferaCore Custom Application Requirements',
                    fields: [
                        { label: 'Company Name', key: 'company_name' },
                        { label: 'Application Name', key: 'application_name' },
                        { label: 'Application Type', key: 'application_type' },
                        { label: 'Expected Number of Users', key: 'number_of_users' },
                        { label: 'Required Modules', key: 'required_modules' },
                        { label: 'Workflow Automation Required?', key: 'workflow_automation' },
                        { label: 'Third-Party API Integrations', key: 'api_integrations' },
                        { label: 'Role-Based Access Control?', key: 'role_based_access' },
                        { label: 'Reports & Dashboard Required?', key: 'reports_dashboard' },
                        { label: 'Additional Requirements', key: 'additional_requirements' }
                    ]
                }
            ];

            let html = '<html><head><title>Lufera Core Wizard</title>';
            html += '<style>';
    ...
    document.querySelectorAll('.load-record').forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            ...
            const fieldMap = {
                ...
                'number_of_users':          data.number_of_users?.value,
                'required_modules':         data.required_modules?.value,
                'workflow_automation':      data.workflow_automation?.value,
                'api_integrations':         data.api_integrations?.value,
                'role_based_access':        data.role_based_access?.value,
                'reports_dashboard':        data.reports_dashboard?.value,
