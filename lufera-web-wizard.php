...
<div class="dashboard-main-body">
    <div class="d-flex flex-wrap align-items-center gap-3 mb-24 justify-content-between">
        <div class="d-flex align-self-end">
            ...
            <h6 class="fw-semibold mb-0">Lufera Web Wizard</h6>
        </div>
...
<?php
    ...
    while ($r = $result->fetch_assoc()) {
        ...
        if ($decoded && !empty($decoded['business_name']['value'] ?? '')) {
            $prevRecords[] = [
                'id'           => $r['id'],
                'data'         => $decoded,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['inline_update'])) {
    ...
    // Find the exact JSON record using:
    ...
    if (
        ...
        // Normal text / select / number / textarea / checkbox / radio
...
<!-- ════════════════════════════════════════════════════════════ -->
<h5>LuferaWeb Website Requirements</h5>

<?php

renderFieldExtended('business_name', $savedData, $user_role, 'Business Name', '', 'text');

renderFieldExtended('website_type', $savedData, $user_role, 'Website Type', '', 'select', [
    'Business Website',
    'Corporate Website',
    'Portfolio Website',
    'Landing Page',
...
renderFieldExtended('number_of_pages', $savedData, $user_role, 'Estimated Number of Pages', '', 'number');

renderFieldExtended('cms_required', $savedData, $user_role, 'CMS Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('contact_form', $savedData, $user_role, 'Contact Form Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('blog_required', $savedData, $user_role, 'Blog Module Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('seo_ready', $savedData, $user_role, 'SEO Ready Website?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('hosting_domain_required', $savedData, $user_role, 'Hosting & Domain Required?', '', 'select', [
    'Yes',
    'No'
]);
...
$('#exportPdfBtn').click(function() {
            ...
            const sections = [
                {
                    title: 'LuferaWeb Website Requirements',
                    fields: [
                        { label: 'Business Name', key: 'business_name' },
                        { label: 'Website Type', key: 'website_type' },
                        { label: 'Business Industry', key: 'business_industry' },
                        { label: 'Estimated Number of Pages', key: 'number_of_pages' },
                        { label: 'CMS Required?', key: 'cms_required' },
                        { label: 'Contact Form Required?', key: 'contact_form' },
                        { label: 'Blog Module Required?', key: 'blog_required' },
                        { label: 'SEO Ready Website?', key: 'seo_ready' },
                        { label: 'Hosting & Domain Required?', key: 'hosting_domain_required' },
                        { label: 'Additional Requirements', key: 'additional_requirements' }
                    ]
                }
            ];

            let html = '<html><head><title>Lufera Web Wizard</title>';
            html += '<style>';
    ...
    document.querySelectorAll('.load-record').forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            ...
            const fieldMap = {
                ...
                'website_type':              data.website_type?.value,
                'business_industry':         data.business_industry?.value,
                'number_of_pages':           data.number_of_pages?.value,
                'cms_required':              data.cms_required?.value,
                'contact_form':              data.contact_form?.value,
                'blog_required':             data.blog_required?.value,
                'seo_ready':                 data.seo_ready?.value,
                'hosting_domain_required':   data.hosting_domain_required?.value,
