...
<div class="dashboard-main-body">
    <div class="d-flex flex-wrap align-items-center gap-3 mb-24 justify-content-between">
        <div class="d-flex align-self-end">
            ...
            <h6 class="fw-semibold mb-0">Hosting Wizard</h6>
        </div>
...
<?php
    ...
    while ($r = $result->fetch_assoc()) {
        ...
        if ($decoded && !empty($decoded['domain_name']['value'] ?? '')) {
            $prevRecords[] = [
                'id'           => $r['id'],
                'data'         => $decoded,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['inline_update'])) {
    ...
    // Find the exact JSON record
    // Same identification used by the normal SAVE logic:
    ...
    // Decode existing JSON if a record exists
    $data = [];
    ...
    // If this field doesn't exist yet, create it
    ...
    if (
        ...
        // Text / select / number / checkbox / radio
...
renderFieldExtended(
    ...
    $savedData,
    $user_role,
    'Business Email Accounts Required',
    '',
    'number'
);
            ...
            const sections = [
                {
                    ...
                    fields: [
                        ...
                        { label: 'Hosting Plan', key: 'hosting_plan' },
                        { label: 'Website Type', key: 'website_type' },
                        { label: 'Storage Required (GB)', key: 'storage_required' },
                        { label: 'Estimated Monthly Visitors', key: 'monthly_visitors' },
                        { label: 'Business Email Accounts Required', key: 'email_accounts' },
                        { label: 'SSL Certificate Required?', key: 'ssl_certificate' },
                        { label: 'Automatic Backup Required?', key: 'backup_required' },
                        { label: 'Website Migration Required?', key: 'migration_required' },
                        { label: 'Additional Requirements', key: 'additional_requirements' }
                    ]
                }
            ];

            let html = '<html><head><title>Hosting Wizard</title>';
            html += '<style>';
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.load-record').forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            ...
            const fieldMap = {
                'domain_name':              data.domain_name?.value,
                'hosting_plan':             data.hosting_plan?.value,
                'website_type':             data.website_type?.value,
                'storage_required':         data.storage_required?.value,
                'monthly_visitors':         data.monthly_visitors?.value,
                'email_accounts':           data.email_accounts?.value,
                'ssl_certificate':          data.ssl_certificate?.value,
                'backup_required':          data.backup_required?.value,
                'migration_required':       data.migration_required?.value,
