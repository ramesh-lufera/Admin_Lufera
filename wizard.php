...
<?php
    ...
    function renderField($fieldName, $savedData, $user_role, $label = '', $placeholder = '') {
        ...
        // if ($isAdmin) {
            if ($status === 'approved') {
                $styleClass = 'field-approved';
            } elseif ($status === 'rejected') {
                $styleClass = 'field-rejected';
            }
        // }

        echo '<div class="input-group">';
        echo '<input type="text" class="form-control ' . $styleClass . '" id="' . $inputId . '" name="' . htmlspecialchars($fieldName) . '" autocomplete="off" placeholder="' . htmlspecialchars($placeholder) . '" value="' . htmlspecialchars($val) . '" ' . $isReadonly . '>';
...
<script>
    $(document).ready(function () {
        ...
        // Enable inline editing for admins
