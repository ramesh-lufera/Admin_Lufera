...
<div class="dashboard-main-body">
    <div class="d-flex flex-wrap align-items-center gap-3 mb-24 justify-content-between">
        <div class="d-flex align-self-end">
            ...
            <h6 class="fw-semibold mb-0">Existing Website Revamp Wizard</h6>
        </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['inline_update'])) {
    ...
    // Find the exact JSON record for this user + website + template
    $jsonId = 0;
    $json_str = null;
    ...
    if (
        ...
        // Normal text / select / radio / checkbox value
        $data[$field]['value'] = $_POST['value'] ?? '';
    }


    // Every admin edit becomes pending
...
<!-- ════════════════════════════════════════════════════════════ -->
<h5>Existing Website Revamp Requirements</h5>

<?php

renderFieldExtended('website_url', $savedData, $user_role, 'Current Website URL', '', 'text');

renderFieldExtended('business_name', $savedData, $user_role, 'Business Name', '', 'text');

renderFieldExtended('current_platform', $savedData, $user_role, 'Current Website Platform', '', 'select', [
    'WordPress',
    'Shopify',
    'Wix',
    'Custom PHP',
...
renderFieldExtended('revamp_scope', $savedData, $user_role, 'Revamp Scope', '', 'select', [
    'UI/UX Redesign',
    'Performance',
    'Full Website',
    'Content Update',
...
renderFieldExtended('pages_to_revamp', $savedData, $user_role, 'Number of Pages', '', 'number');

renderFieldExtended('responsive_design', $savedData, $user_role, 'Mobile Responsive Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('seo_optimization', $savedData, $user_role, 'SEO Optimization Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('content_update', $savedData, $user_role, 'Content Update Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('deadline', $savedData, $user_role, 'Expected Completion Date', '', 'date');
...
$('#exportPdfBtn').click(function() {
            ...
            const sections = [
                {
                    title: 'Existing Website Revamp Requirements',
                    fields: [
                        { label: 'Current Website URL', key: 'website_url' },
                        { label: 'Business Name', key: 'business_name' },
                        { label: 'Current Website Platform', key: 'current_platform' },
                        { label: 'Revamp Scope', key: 'revamp_scope' },
                        { label: 'Number of Pages', key: 'pages_to_revamp' },
                        { label: 'Mobile Responsive Required?', key: 'responsive_design' },
                        { label: 'SEO Optimization Required?', key: 'seo_optimization' },
                        { label: 'Content Update Required?', key: 'content_update' },
                        { label: 'Expected Completion Date', key: 'deadline' },
                        { label: 'Additional Requirements', key: 'additional_requirements' }
                    ]
                }
            ];

            let html = '<html><head><title>Existing Website Revamp Wizard</title>';
            html += '<style>';
            ...
            html += '<h1>Existing Website Revamp Wizard</h1>';
    ...
    document.querySelectorAll('.load-record').forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            ...
            const fieldMap = {
                ...
                'current_platform':        data.current_platform?.value,
                'revamp_scope':            data.revamp_scope?.value,
                'pages_to_revamp':         data.pages_to_revamp?.value,
                'responsive_design':       data.responsive_design?.value,
                'seo_optimization':        data.seo_optimization?.value,
                'content_update':          data.content_update?.value,
