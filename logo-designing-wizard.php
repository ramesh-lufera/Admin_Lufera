...
<div class="dashboard-main-body">
    <div class="d-flex flex-wrap align-items-center gap-3 mb-24 justify-content-between">
        <div class="d-flex align-self-end">
            ...
            <h6 class="fw-semibold mb-0">Logo Designing Wizard</h6>
        </div>
...
<?php
    ...
    while ($r = $result->fetch_assoc()) {
        ...
        if ($decoded && !empty($decoded['company_name']['value'] ?? '')) {
            $prevRecords[] = [
                'id'           => $r['id'],
                'data'         => $decoded,
...
//   UPDATE existing json record OR INSERT new one
// ────────────────────────────────────────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['inline_update'])) {
    ...
    // ------------------------------------------------
    // Get the current JSON record, if one exists
    // ------------------------------------------------
    $data = [];
    $recordExists = false;

    $stmt = $conn->prepare("
        SELECT id, name, user_id, template
        FROM json
        WHERE website_id = ?
        LIMIT 1
    ");
    ...
    if ($result->num_rows > 0) {
        ...
        $data = json_decode($existingRow['name'], true) ?? [];

        // Keep these values for update/insert
        $json_user_id = intval($existingRow['user_id']);
        $json_template = $existingRow['template'];
    }
    ...
    // ------------------------------------------------
    // If record exists, make sure the field exists.
    // If record does NOT exist, create the field.
    // ------------------------------------------------
    if (!isset($data[$field])) {
        $data[$field] = [
            'value' => '',
    ...
    if (
        ...
        // Normal text/select/textarea/radio/checkbox value
        $value = $_POST['value'] ?? '';
    ...
    if ($recordExists) {
        ...
        // ------------------------------------------------
        // INSERT new record when no json row exists
        // ------------------------------------------------

        // Use the current target user and template.
        ...
        if ($insert->execute()) {
            ...
            http_response_code(500);
            echo 'database insert failed: ' . $insert->error;
        }
...
<!-- ════════════════════════════════════════════════════════════ -->
<h5>Logo Designing Requirements</h5>

<?php

renderFieldExtended('company_name', $savedData, $user_role, 'Company / Brand Name', '', 'text');

renderFieldExtended('logo_style', $savedData, $user_role, 'Preferred Logo Style', '', 'select', [
    'Minimalist',
    'Modern',
    'Luxury',
    'Corporate',
...
renderFieldExtended('business_industry', $savedData, $user_role, 'Business Industry', '', 'text');

renderFieldExtended('brand_colors', $savedData, $user_role, 'Preferred Brand Colors', '', 'text');

renderFieldExtended('tagline', $savedData, $user_role, 'Tagline (Optional)', '', 'text');

renderFieldExtended('logo_usage', $savedData, $user_role, 'Logo Usage', '', 'textarea');

renderFieldExtended('reference_designs', $savedData, $user_role, 'Reference Logo / Inspiration', '', 'textarea');

renderFieldExtended('deliverables', $savedData, $user_role, 'Required Deliverables', '', 'select', [
    'PNG',
    'JPG',
    'SVG',
    'AI',
...
renderFieldExtended('number_of_concepts', $savedData, $user_role, 'Logo Concepts Required', '', 'select', [
    '1',
    '2',
    '3',
    '5'
...
renderFieldExtended('additional_requirements', $savedData, $user_role, 'Additional Requirements', '', 'textarea');
...
<?php if (in_array($user_role, [8])): ?>
    <input type="submit" id="saveBtn" name="save" class="lufera-bg lufera-text text-md px-56 py-11 radius-8 m-auto d-block mt-5" value="Save">
<?php endif; ?>
...
function updateProgressBar() {
    let filled = 0;
    const total = 10; // adjust if you add/remove fields
...
$('#exportPdfBtn').click(function() {
            ...
            const sections = [
                {
                    title: 'Logo Designing Requirements',
                    fields: [
                        { label: 'Company / Brand Name', key: 'company_name' },
                        { label: 'Preferred Logo Style', key: 'logo_style' },
                        { label: 'Business Industry', key: 'business_industry' },
                        { label: 'Preferred Brand Colors', key: 'brand_colors' },
                        { label: 'Tagline (Optional)', key: 'tagline' },
                        { label: 'Logo Usage', key: 'logo_usage' },
                        { label: 'Reference Logo / Inspiration', key: 'reference_designs' },
                        { label: 'Required Deliverables', key: 'deliverables' },
                        { label: 'Logo Concepts Required', key: 'number_of_concepts' },
                        { label: 'Additional Requirements', key: 'additional_requirements' }
                    ]
                }
            ];

            let html = '<html><head><title>Logo Designing Wizard</title>';
            html += '<style>';
            ...
            html += '<h1>Logo Designing Wizard</h1>';
    ...
    document.querySelectorAll('.load-record').forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            ...
            const fieldMap = {
                ...
                'logo_style':               data.logo_style?.value,
                'business_industry':        data.business_industry?.value,
                'brand_colors':             data.brand_colors?.value,
                'tagline':                  data.tagline?.value,
                'logo_usage':               data.logo_usage?.value,
                'reference_designs':        data.reference_designs?.value,
                'deliverables':             data.deliverables?.value,
                'number_of_concepts':       data.number_of_concepts?.value,
                'additional_requirements':  data.additional_requirements?.value
            };
