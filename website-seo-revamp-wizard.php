...
<div class="dashboard-main-body">
    <div class="d-flex flex-wrap align-items-center gap-3 mb-24 justify-content-between">
        <div class="d-flex align-self-end">
            ...
            <h6 class="fw-semibold mb-0">Website SEO Revamp Wizard</h6>
        </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['inline_update'])) {
    ...
    // Find existing record using the same identifiers
    // used by the normal SAVE logic.
    $jsonId = 0;
    $jsonStr = '';
    ...
    // If record exists, load its JSON.
    // If no record exists, start with an empty JSON object.
    $data = [];
    ...
    // create it instead of returning "field not found".
    ...
    if (
        ...
        // Normal text / select / textarea / checkbox / radio
...
<!-- ════════════════════════════════════════════════════════════ -->
<h5>Website SEO Revamp Requirements</h5>
...
renderFieldExtended('target_keywords', $savedData, $user_role, 'Target Keywords', '', 'textarea');

renderFieldExtended('target_locations', $savedData, $user_role, 'Target Locations', '', 'textarea');

renderFieldExtended('competitor_websites', $savedData, $user_role, 'Competitor Websites', '', 'textarea');

renderFieldExtended('technical_seo', $savedData, $user_role, 'Technical SEO Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('onpage_seo', $savedData, $user_role, 'On-Page SEO Optimization?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('monthly_reports', $savedData, $user_role, 'Monthly SEO Reports Required?', '', 'select', [
    'Yes',
    'No'
]);
...
$('#exportPdfBtn').click(function() {
            ...
            const sections = [
                {
                    title: 'Website SEO Revamp Requirements',
                    fields: [
                        { label: 'Website URL', key: 'website_url' },
                        { label: 'Business Name', key: 'business_name' },
                        { label: 'Business Industry', key: 'business_industry' },
                        { label: 'Target Keywords', key: 'target_keywords' },
                        { label: 'Target Locations', key: 'target_locations' },
                        { label: 'Competitor Websites', key: 'competitor_websites' },
                        { label: 'Technical SEO Required?', key: 'technical_seo' },
                        { label: 'On-Page SEO Optimization?', key: 'onpage_seo' },
                        { label: 'Monthly SEO Reports Required?', key: 'monthly_reports' },
                        { label: 'Additional Requirements', key: 'additional_requirements' }
                    ]
                }
            ];

            let html = '<html><head><title>Website SEO Revamp Wizard</title>';
            html += '<style>';
            ...
            html += '<h1>Website SEO Revamp Wizard</h1>';
    ...
    document.querySelectorAll('.load-record').forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            ...
            const fieldMap = {
                ...
                'target_keywords':         data.target_keywords?.value,
                'target_locations':        data.target_locations?.value,
                'competitor_websites':     data.competitor_websites?.value,
                'technical_seo':           data.technical_seo?.value,
                'onpage_seo':              data.onpage_seo?.value,
                'monthly_reports':         data.monthly_reports?.value,
