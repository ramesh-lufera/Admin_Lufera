...
<div class="dashboard-main-body">
    <div class="d-flex flex-wrap align-items-center gap-3 mb-24 justify-content-between">
        <div class="d-flex align-self-end">
            ...
            <h6 class="fw-semibold mb-0">Lufera Sheets Wizard</h6>
        </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['inline_update'])) {
    ...
    // If no DB row exists, start with an empty array
    $data = [];
...
<!-- ════════════════════════════════════════════════════════════ -->
<h5>LuferaSheets CRM Requirements</h5>
...
renderFieldExtended('industry_type', $savedData, $user_role, 'Industry Type', '', 'text');

renderFieldExtended('lead_sources', $savedData, $user_role, 'Lead Sources', '', 'textarea');

renderFieldExtended('crm_users', $savedData, $user_role, 'Number of CRM Users', '', 'number');

renderFieldExtended('lead_pipeline', $savedData, $user_role, 'Lead Pipeline Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('custom_forms', $savedData, $user_role, 'Custom Forms Required?', '', 'select', [
    'Yes',
    'No'
]);

renderFieldExtended('email_notifications', $savedData, $user_role, 'Email Notifications', '', 'select', [
    'Enabled',
    'Disabled'
]);

renderFieldExtended('reports_required', $savedData, $user_role, 'Reports & Dashboard Required?', '', 'select', [
    'Yes',
    'No'
]);
...
$('#exportPdfBtn').click(function() {
            ...
            const sections = [
                {
                    title: 'LuferaSheets CRM Requirements',
                    fields: [
                        { label: 'Business Name', key: 'business_name' },
                        { label: 'Website URL', key: 'website_url' },
                        { label: 'Industry Type', key: 'industry_type' },
                        { label: 'Lead Sources', key: 'lead_sources' },
                        { label: 'Number of CRM Users', key: 'crm_users' },
                        { label: 'Lead Pipeline Required?', key: 'lead_pipeline' },
                        { label: 'Custom Forms Required?', key: 'custom_forms' },
                        { label: 'Email Notifications', key: 'email_notifications' },
                        { label: 'Reports & Dashboard Required?', key: 'reports_required' },
                        { label: 'Additional Requirements', key: 'additional_requirements' }
                    ]
                }
            ];

            let html = '<html><head><title>Lufera Sheets Wizard</title>';
            html += '<style>';
            ...
            html += '<h1>Lufera Sheets Wizard</h1>';
    ...
    document.querySelectorAll('.load-record').forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            ...
            const fieldMap = {
                ...
                'industry_type':            data.industry_type?.value,
                'lead_sources':             data.lead_sources?.value,
                'crm_users':                data.crm_users?.value,
                'lead_pipeline':            data.lead_pipeline?.value,
                'custom_forms':             data.custom_forms?.value,
                'email_notifications':      data.email_notifications?.value,
                'reports_required':         data.reports_required?.value,
